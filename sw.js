// ★★★ バージョン表記（サイト全体で唯一）。手で変更する必要はない ★★★
// GitHubにアップすると GitHub Actions（.github/workflows/deploy.yml）が
// この値を「日付-コミットID」に自動で書き換えてから公開する。
// 値が変わると次回アクセス時に全ファイルが新しいキャッシュへ一括で取り直され、
// 自動リロードで新バージョンが反映される（古いキャッシュは自動削除）。
// ※ローカル確認用の値なので、ここの文字列は何でもよい。
const SW_VERSION = '20260906e';
