...
/* ===== FINAL STABILIZER (idempotent) ===== */
(function(){
  if (window.__finalPatchApplied) return; window.__finalPatchApplied = true;

  // 1) vh unit fix for iOS Safari toolbars
  function setVH(){
  ...
  // 2) Recompute --header-offset from actual sticky header height
  function updateHeaderOffset(){
    var s = document.querySelector('.sticky-search-area');
    if (!s) return;
  ...
  // Observe header mutations (tags added/removed)
  if ('MutationObserver' in window){
    var s = document.querySelector('.sticky-search-area');
    if (s){
      var mo = new MutationObserver(function(){ setTimeout(updateHeaderOffset, 0); });
      mo.observe(s, {childList:true, subtree:true, attributes:true});
    }
  }

  // 3) Unified scroll-lock helpers (class-based, body top trick)
  var __scrollY = 0;
  function lockScroll(){
    if (document.body.classList.contains('scroll-lock')) return;
    __scrollY = window.scrollY || document.documentElement.scrollTop || 0;
  ...
  function unlockScroll(){
    ...
    document.body.style.top = '';
    try{ window.scrollTo(0, top ? -parseInt(top,10) : __scrollY); }catch(e){}
  }
  window.acquireBodyLock = lockScroll;
  window.releaseBodyLock = unlockScroll;

  // 4) Stable drawer positioning + handlers
  function positionDrawer(){
    var sbar = document.querySelector('.sticky-search-area');
    var drawer = document.getElementById('filterDrawer');
    if (!sbar || !drawer) return;
    var rect = sbar.getBoundingClientRect();
    drawer.style.position = 'fixed';
    drawer.style.left = '50%';
    drawer.style.transform = 'translateX(-50%)';
    drawer.style.right = 'auto';
    drawer.style.top = (rect.top + rect.height + 8) + 'px';
    // height cap based on viewport
    var winH = window.innerHeight;
    var maxH = Math.max(240, winH - (rect.top + rect.height + 24));
    drawer.style.maxHeight = maxH + 'px';
  }

  function openDrawer(){
    positionDrawer();
    $('#filterDrawer').stop(true,true).fadeIn(90);
    $('#drawerBackdrop').addClass('show');
    $('#filterToggleBtn').attr({'aria-expanded':true, 'aria-pressed':true});
    lockScroll();
  }
  function closeDrawer(){
    $('#filterDrawer').stop(true,true).fadeOut(80, function(){
      $('#drawerBackdrop').removeClass('show');
    });
    $('#filterToggleBtn').attr({'aria-expanded':false, 'aria-pressed':false});
    unlockScroll();
  ...
  // Remove any pre-existing handlers that might conflict, then bind once
  $('#filterToggleBtn').off('.__final').on('click.__final keypress.__final', function(e){
    if (e.type === 'click' || (e.type==='keypress' && (e.key==='Enter' || e.key===' '))){
      $('#filterDrawer').is(':visible') ? closeDrawer() : openDrawer();
    }
  });
  $('#drawerBackdrop').off('.__final').on('click.__final', closeDrawer);
  $(document).off('.__final_outside').on('click.__final_outside', function(e){
    if ($('#filterDrawer').is(':visible') && !$(e.target).closest('#filterDrawer,#filterToggleBtn').length){
      closeDrawer();
    }
  });
  ...
  // Safety: if the drawer becomes hidden by any path, unlock body
  (function(){
    var drawer = document.getElementById('filterDrawer');
    if (!drawer || window.__drawerObserverSetupFinal) return;
    window.__drawerObserverSetupFinal = true;
