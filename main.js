...
(function () {
  ...
  function syncDrawerHeaderOffset(){
    const header = drawer.querySelector('.filter-drawer-header');
    const hasHeader = !!header;
    drawer.classList.toggle('has-header', hasHeader);

    // ヘッダーがあるなら実測値、無いなら0px
    let h = 0;
    if (hasHeader) {
      // 実測（境界線含む）
      const rect = header.getBoundingClientRect();
      h = Math.max(0, Math.round(rect.height));
      if (!h) h = 56; // 万一計測0なら既定値
    }
    drawer.style.setProperty('--drawer-header-h', `${h}px`);
  }

  // 初回
  syncDrawerHeaderOffset();

  // Drawerを開くたび＆リサイズ時も同期（高さが動くUIの保険）
  const toggle = document.getElementById('filterToggleBtn');
  if (toggle) toggle.addEventListener('click', () => setTimeout(syncDrawerHeaderOffset, 0));
  window.addEventListener('resize', () => syncDrawerHeaderOffset(), { passive: true });
})();
...
// 固定ヘッダーの実高を測って CSS 変数へ反映
(function(){
  const root = document.documentElement;
  const sticky = document.querySelector('.sticky-search-area');

  function updateHeaderOffset(){
    // 固定ヘッダーの実高さ（影や下段のバーを含めて）を取得
    const h = sticky ? sticky.getBoundingClientRect().height : 110;
    root.style.setProperty('--header-offset', `${Math.round(h)}px`);
  }

  // 初期/リサイズ/向き変更/フォントロード後に反映
  ['load','resize','orientationchange'].forEach(ev =>
    window.addEventListener(ev, updateHeaderOffset, { passive: true })
  );
  // 初期実行（DOMContentLoaded より前でも OK）
  updateHeaderOffset();

  // Drawer を開く前後でも再計算（高さが変動するサイトのため）
  const toggle = document.getElementById('filterToggleBtn');
  if (toggle) toggle.addEventListener('click', () => setTimeout(updateHeaderOffset, 0));
})();


// --- Fix: 適用で確実にロック解除して閉じる（古い委譲を止める） ---
$(function () {
  ...
  $('#drawerApplyBtn')
    ...
    .on('click keypress', function (e) {
      ...
      if (typeof search === 'function') search();   // 検索実行
      if (window.__closeDrawer) window.__closeDrawer(); // ← 統一クローズ（unlock込）
      if (typeof updateScrollLock === 'function') updateScrollLock(); // 念押し
      e.stopImmediatePropagation();  // ↓ 古い document への伝播を止める
      e.stopPropagation();
      e.preventDefault();
    });

  // （任意）「すべてクリア」は Drawer を閉じない。古い委譲を止めるだけ。
  $('#drawerClearBtn')
    .off('click keypress')
    .on('click keypress', function (e) {
      if (!isActivateEvent(e)) return;
      // 既存の clear ロジックがあればそれを呼ぶ／無ければトグル解除→search()
      const drawer = document.getElementById('filterDrawer');
      if (drawer) {
